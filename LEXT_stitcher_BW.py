...
def get_cols_rows_from_filename(path):
    ...
    for file in path:
        # print(file.split("\\")[-1][8:11])
        # print(file.split("\\")[-1][-9:-6])
        cols.append(int(file.split("\\")[-1][8:11]))
        rows.append(int(file.split("\\")[-1][-9:-6]))
...
def sample_total_fitting(files):
    ...
    if side.is_integer():
        ...
        diagonal = np.linspace(0,len(files)-1,int(side))[1:-1]
        # diagonal = [166,182,182,15,30,45,60,75,90]
        if False:## Take average of cross, not just diagonal
            diagonal = np.append(diagonal,[side*i-i for i in range(1,int(side))])
            # diagonal = [side*i-i for i in range(2,int(side))]

        for cf, f in enumerate(files):
            if cf in diagonal:
                print(f"Pic {cf} fitted")
                fit = quantify_image(f)
                # fit = resize_matrix
                collected.append(fit)
    else:
...
folder_path = "D:\\PerfectStitch\\original\\"
...
collected = sample_total_fitting(img_array)
united=np.mean(collected,axis=0).astype("uint8")
...
# fixed_pics = []
# bright = np.max(united)+1
# folder = "C:\\Users\\HYDN02\\Seafile\\Code\\DataExamples\\stitch_flat\\"
# for n, f in enumerate(img_array):
#     new_img = f-united+bright
#     cv2.imwrite(folder+"X"+str(cols[n])+"_Y"+str(rows[n])+".jpg", new_img)
